# -*- coding: utf-8 -*-
"""lichess_opening_analysis-black version.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/15_DsyVzkffgEp0yyAuMFkoO7S8t5-v4A
"""
...
with open("/content/drive/MyDrive/compressed_pgn_data.json") as f:
    ...
    # but now it automatically opens
print(len(pgn_file))
main_list = pgn_file
print(main_list[10])
...
for i in range(len(pgn_file)):
      ...
      for j in range(len(rating_categorisation)):
        if rating_categorisation[j-1] < int(black_rating) < rating_categorisation[j]:
          black_rating_band = rating_categorisation[j-1]
          break


      if int(black_rating) > 2100:
        black_rating_band = 2100
      if opening not in opening_lists[black_rating_band]:
        all_openings[black_rating_band][opening] = 1
      
      elif opening in opening_lists[black_rating_band]:
      
        all_openings[black_rating_band][opening] += 1

      if result == "0-1" and opening not in opening_lists[black_rating_band]:
        opening_wins[black_rating_band][opening] = 1

      elif result == "0-1" and opening in opening_lists[black_rating_band]:
        opening_wins[black_rating_band][opening] += 1  # adds the openings to 
      ...
      if opening not in opening_lists[black_rating_band]:
        opening_lists[black_rating_band].append(opening)
...
dubious_openings = ["?", "Crab", "Amazon", "Sodium", "Grob", "Bird"]
  ...
  for i in range(len(rating_categorisation)):
    ...
    for j in range(len(best_openings_list[rating_categorisation[i]])):
      try:
        ...
        if all_openings[rating_categorisation[i]][best_openings_list[rating_categorisation[i]][j]] < 200:
...
def graph_maker(largest, win_percent, range, lims):
  ...
  plt.ylim(lims)
  plt.title(f"Most Successful Openings for Black, between {range}")  
  plt.ylabel("Win percentage for Black")
...
results_cloggers = ["?", "Crab", "Sodium", "Amazon", "Grob"] 
